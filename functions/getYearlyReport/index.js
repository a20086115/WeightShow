...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 查询该年度的所有打卡记录（使用分页查询获取所有数据）
    const startDate = `${year}-01-01`;
    const endDate = `${year}-12-31`;
    
    // 微信云开发云函数端单次查询最多返回100条，需要分页查询获取所有数据
    const MAX_LIMIT = 100; // 微信云开发云函数端单次查询最大限制
    const MAX_RECORDS = 500; // 安全限制：最多查询500条记录（一年最多365天）
    let recordsData = [];
    let hasMore = true;
    let skip = 0;
    
    while (hasMore && recordsData.length < MAX_RECORDS) {
      ...
      const batchData = records.data || [];
      
      if (batchData.length > 0) {
        recordsData = recordsData.concat(batchData);
    ...
    // 记录查询到的数据量，便于调试
    console.log(`查询到 ${recordsData.length} 条年度记录`);
    ...
    if (weightRecords.length > 0) {
      ...
      recordsData.forEach(record => {
        ...
        if (!monthlyData[month]) {
          monthlyData[month] = {
            ...
            weightCount: 0,
            weights: [],
            weightRecords: [] // 保存带日期的体重记录，用于计算月度减重
          };
        }
        ...
        if (record.weight && record.weight > 0) {
          ...
          // 保存带日期的体重记录
          monthlyData[month].weightRecords.push({
            date: record.date,
      ...
      Object.keys(monthlyData).forEach(month => {
        ...
        if (monthData.weights.length > 0) {
          ...
          // 按时间顺序计算月度减重：该月最早体重 - 该月最晚体重
          let weightLoss = 0;
          if (monthData.weightRecords.length > 1) {
            // 按日期排序
            const sortedRecords = monthData.weightRecords.sort((a, b) => {
              return a.date.localeCompare(b.date);
            });
            const firstWeight = sortedRecords[0].weight; // 该月最早体重
            const lastWeight = sortedRecords[sortedRecords.length - 1].weight; // 该月最晚体重
            weightLoss = firstWeight - lastWeight; // 正数表示减重，负数表示增重
          }
          ...
          stats.monthlyStats[month] = {
            ...
            weightLoss: parseFloat(weightLoss.toFixed(2)) // 按时间顺序计算的减重
          };
          
          // 找出减重最多的月份（逆袭月）- 只考虑实际减重（正数）
          if (weightLoss > 0 && weightLoss > maxWeightLoss) {
