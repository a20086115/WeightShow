...
// 重新计算用户的连续打卡天数（用于删除记录后）
async function recalculateUserContinuousDays(db, openId) {
  try {
    ...
    if (allDates.length === 0) {
      ...
      return { continuousDays: 0, lastCheckInDate: null }
    }
    
    const lastCheckInDate = allDates[0] // 最新的打卡日期
    
    // 计算连续打卡天数
    const continuousDays = calculateContinuousDays(allDates, lastCheckInDate)
    ...
    console.log(`[${openId}] 重新计算连续打卡天数: ${continuousDays}, 最后打卡日期: ${lastCheckInDate}`)
    ...
    console.error(`重新计算用户连续打卡天数失败 [${openId}]:`, error)
    throw error
  }
}
...
exports.main = async (event, context) => {
  try {
    ...
    // 执行删除
    const result = await db.collection(tbName).where(query).remove();

    // 如果是删除records记录，重新计算用户的连续打卡天数
    if (tbName === 'records') {
      try {
        await recalculateUserContinuousDays(db, openId);
      } catch (error) {
    ...
    console.error('delete云函数执行失败:', error);
    return {
